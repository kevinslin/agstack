...
- When persistent heartbeat automation is unavailable and the user explicitly asks the active task to watch the auto-merge-pending PR until completion, use a foreground watch instead of ending with partial finalization. Treat this as active monitoring, never as an unattended handoff. Retain the locked repository, PR number, head branch, and exact head SHA; poll the same PR live with no more than 60 seconds between checks; and publish one concise progress update at least every 60 seconds.
