...
def validate_sources(raw_sources: list[str]) -> tuple[list[Path], str | None]:
    ...
    for raw_source in raw_sources:
        ...
        if source.is_symlink():
            return [], f"source path must not be a symlink: {source}"
        try:
            resolved = source.resolve(strict=True)
        except OSError:
            return [], f"source path does not exist: {source}"
        if not resolved.is_file() and not resolved.is_dir():
            return [], f"source path is not a regular file or directory: {resolved}"
        label = str(resolved)
...
def iter_files(
    ...
    *,
    ...
    for directory, dirnames, filenames in os.walk(scope, topdown=True, followlinks=False):
        ...
        for dirname in sorted(dirnames):
            ...
            elif dirname.startswith(".") or dirname in SKIP_DIRECTORIES:
...
def read_text(path: Path, stats: dict[str, Any]) -> str | None:
...
def search_managed(
    ...
    for base in bases:
        ...
        for path in iter_files(managed_root, stats=stats, area="managed"):
...
def search_sources(
    ...
    for source in sources:
        ...
        for path in iter_files(source, stats=stats, area="source", seen=seen):
...
def main() -> None:
    ...
    result["status"] = "matched" if result["source_matches"] else "no_matches"
    emit(result, pretty=args.pretty)
