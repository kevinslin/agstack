...
- [fin](active/fin/SKILL.md): Finalize a completed task through a selected `gh` or `local` landing flow, using a mandatory origin-main gate plus a deduplicated heartbeat or explicit foreground watch that resumes deferred cleanup only after live verification of a pending auto-merge.
