...
- [specy](active/specy/SKILL.md): Write focused specs, flow docs, and concise logic trace cheatsheets with compact ownership and scope checkpoints plus a digest-verified feature-spec simplification gate.
- [dev.review](active/dev.review/SKILL.md): Review code and specs against current decisions and ownership, with consumer-backed, contract-preserving simplification, decision-ready recommendations, mandatory evidence-backed simplicity verdicts and test audits, explicit keep/merge/delete test decisions, ordinary user-path proof, and removal of useless tests.
...
- [dev.shortcuts](active/dev.shortcuts/SKILL.md): Route focused workflows, enforce fresh specification-simplification verdicts, and make duplicate ownership or parallel implementations actionable review findings.
...
- [sw-loop](active/sw-loop/SKILL.md): Swarm-based feature delivery that preserves settled decisions and approved capabilities while requiring explicit simplification review, evidence-backed current-head simplicity and test audits, removal of tests without distinct production outcomes, a post-fix minimization gate, evidence-aware local verification, delta-focused follow-up reviews, documentation, and a separate push gate.
...
- [dev.codex](active/dev.codex/SKILL.md): Codex runtime/config management, including MCP servers, remote saved project registration, session setup, and session summaries. Use `$sc` for skill authoring.
...
- [dev.loop](active/dev.loop/SKILL.md): Drive development through infrastructure-aware acceptance planning, reusable verification evidence, change-scoped reruns, and separate push/PR/CI delivery without weakening required real-world proof.
- [dev.review](active/dev.review/SKILL.md): Review code, docs, specs, architecture, and UX against current decisions and ownership, with consumer-backed, contract-preserving simplification, decision-ready recommendations, ordinary user-path proof, mandatory evidence-backed simplicity verdicts and test audits; classify every changed test as keep, merge, or delete; and treat duplicate authority, parallel implementations, and useless tests as major while preserving approved capabilities.
- [dev.shortcuts](active/dev.shortcuts/SKILL.md): Resolve explicit `trigger:<name>` workflows, including digest-verified specification-simplification completion gates; evidence-gated review/fix loops with delta-focused follow-ups; ready-for-review PR creation; stacked-PR-safe merge and branch cleanup; and task archival.
...
- [fin](active/fin/SKILL.md): Finalize PR or local work with a default-branch gate, downstream-PR protection, and verified cleanup; use explicit `fin force [target]` to waive only missing PR approvals with existing permissions, or `fin nocheck` for local review cleanup.
...
- [specy](active/specy/SKILL.md): Create research documentation, source-backed flow docs, sparse logic trace cheatsheets, and focused feature specs with mandatory scope, ownership, invariant, and operational-cost checkpoints plus a digest-verified simplification gate; reject oversized specs without justification, preserve concrete contracts and outcome-to-proof verification, and keep archival link-safe. Route durable artifacts through `$mem` and use `$docy` `ref/spec` when writing specifications.
